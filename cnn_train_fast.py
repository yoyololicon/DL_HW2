...
def main(_):
    with tf.Session() as sess:
        ...
        with tf.variable_scope('train'):
            ...
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=outputs), name='loss_op')
        ...
        for i in range(epochs):
            ...
            for j in range(val_num // batch_size + 1):
                ...
                loss_value, acc_value = sess.run([loss_tensor, acc_op],
                                                 feed_dict={X: val_x[pos:pos + nums], Y: val_y[pos:pos + nums]})
